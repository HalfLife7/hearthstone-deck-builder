...
import DeckItem from "../components/DeckItem.jsx";
...
describe('DeckItem', () => {
    it('displays the card name', async () => {
        render(<DeckItem card={card}/>)

        expect(screen.getByText('Ragnaros the Firelord')).toBeInTheDocument()
    })

    it('displays the mana cost', () => {
        render(<DeckItem card={card}/>)

        expect(screen.getByAltText(`Mana: ${card.cost}`))
    })
})